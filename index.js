...
doc.text(m.department, 250, currentY+5, { width: 150 });
...
if (!isIndividual) doc.text('Sardor imzosi: ____________________', { align: 'right' });
...
async function showDepartments(chatId, callbackDataPrefix) {
const buttons = [];
for (let i = 0; i < DEPARTMENTS_LIST.length; i += 2) {
const row = [];
row.push({ text: DEPARTMENTS_LIST[i], callback_data: ${callbackDataPrefix}_${DEPARTMENTS_LIST[i]} });
if (i+1 < DEPARTMENTS_LIST.length) row.push({ text: DEPARTMENTS_LIST[i+1], callback_data: ${callbackDataPrefix}_${DEPARTMENTS_LIST[i+1]} });
buttons.push(row);
}
...
members: members.map((m, idx) => ({ name: m.name, department: m.department, role: idx === 0 ? 'Sardor' : 'A'zo' })),
createdBy: userId,
...
captainName: members[0].name,
captainDepartment: members[0].department,
members: members.map(m => ({ name: m.name, department: m.department }))
}, false);

await bot.sendDocument(chatId, pdfBuffer, { filename: ariya_${newTeam.teamId}.pdf, contentType: 'application/pdf', caption: ✅ "${teamName}" jamoasi muvaffaqiyatli ro'yxatdan o'tdi!\n\n📄 Ariza faylingiz. Iltimos, uni imzolab Yoshlar kengashiga topshiring. });
...
const pdfBuffer = await generateApplicationPDF({ name, department, registeredAt: newIndividual.registeredAt }, true);
...
"📌 Jamoa bilan ro'yxatdan o'tish: 5 kishidan iborat jamoa tuzasiz (sardor + 4 a'zo). Har bir a'zo uchun bo'lim va to'liq ism kiritiladi.\n" +
"📌 Individual ro'yxatdan o'tish: Jamoasi bo'lmagan ishtirokchilar uchun. Siz faqat o'z bo'limingiz va ismingizni kiritasiz.\n" +
...
[{ text: "📁 Barcha arizalarni yuklab olish (ZIP)", callback_data: "admin_export_all_pdfs" }],
[{ text: db.registrationOpen ? "🔒 Ro'yxatni yopish" : "🔓 Ro'yxatni ochish", callback_data: "admin_toggle_registration" }]
]
}
};
await bot.sendMessage(chatId, stats, adminButtons);
});
...
members: team.members.map(m => ({ name: m.name, department: m.department }))
}, false);
archive.append(pdfBuffer, { name: jamoa_${team.teamId}.pdf });
}
for (const ind of db.individuals) {
const pdfBuffer = await generateApplicationPDF({ name: ind.name, department: ind.department, registeredAt: ind.registeredAt }, true);
...
// JAMOA YARATISH: bo'lim tanlash (sardor yoki a'zo)
if (data.startsWith('team_captain_dept_') || data.startsWith('team_member_dept_')) {
const parts = data.split('');
const role = parts[1]; // 'captain' yoki 'member'
const department = parts.slice(3).join('');
...
const newSession = { ...session, currentDepartment: department };
if (role === 'captain') {
newSession.currentRole = 'captain';
await askMemberName(chatId, newSession, 1);
} else {
newSession.currentRole = 'member';
...
await showDepartments(chatId, 'individual_dept');
return;
}
...
members: userTeam.members.map(m => ({ name: m.name, department: m.department }))
}, false);
...
await showDepartments(chatId, 'team_captain_dept');
return;
}
